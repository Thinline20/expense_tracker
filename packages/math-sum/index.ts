...
export function sum(iterable: Iterable<number>): number {
  ...
  let overflow: number = 0; // conceptually 2**1024 times this value; the final partial
  ...
  let next: () => IteratorResult<number> = iterator.next.bind(iterator);

  // in C this would be done using a goto
  function drainNonFiniteValue(current: number): number {
    while (true) {
      let { done, value } = next();
      if (done) {
        return current;
  ...
  while (true) {
    ...
    let actuallyUsedPartials: number = 0;
    // let newPartials = [];
    for (let y of partials) {
      ...
      if (Math.abs(hi) === Infinity) {
        let sign: number = hi === Infinity ? 1 : -1;
        overflow += sign;
  ...
  let n: number = partials.length - 1;
  let hi: number = 0;
  let lo: number = 0;

  if (overflow !== 0) {
    let next: number = n >= 0 ? partials[n] : 0;
    --n;
